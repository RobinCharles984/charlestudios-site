...
export function ProjectDetails() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/projects/${slug}`)
      .then(res => { 
        // 🛡️ Normalização de Dados (Para não quebrar com posts antigos)
        const data = res.data;
        if (!data.types) {
           // Se não tiver 'types' (array), usa o 'type' antigo ou define padrão
  ...
  const isStudy = project.types.includes('study'); // Verifica se é estudo
    ...
    <div className="min-h-screen bg-slate-900 text-white font-sans pb-10">
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div>
            ...
            {/* 5. Links Externos (Incluindo ArtStation) */}
            <div className="flex flex-wrap gap-4 mb-12 border-t border-slate-800 pt-8">
              ...
              {project.artstationLink && (
                  <a href={project.artstationLink} target="_blank" rel="noreferrer" className="px-6 py-3 bg-[#13aff0] hover:bg-[#0b92cc] rounded-lg transition font-bold flex items-center gap-2 text-white">
                    🎨 ArtStation
                  </a>
              )}
